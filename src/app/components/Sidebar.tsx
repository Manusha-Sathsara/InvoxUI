...
import type { ViewType, Tenant, AppUser } from '../App'
...
export function Sidebar({
  isDark, currentView, onViewChange, currentTenant, onTenantChange,
  currentUser, onUserChange, collapsed, onCollapsedChange, mobileOpen, onLogout,
}: SidebarProps) {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (
        tenantBtnRef.current && !tenantBtnRef.current.contains(target) &&
        tenantPortalRef.current && !tenantPortalRef.current.contains(target)
      ) setTenantDropOpen(false)
      if (
        userBtnRef.current && !userBtnRef.current.contains(target) &&
        userPortalRef.current && !userPortalRef.current.contains(target)
  ...
  return (
    ...
    <div className={sidebarBase}>
      ...
      <div className={`flex items-center px-4 h-16 border-b ${isDark ? 'border-white/[0.06]' : 'border-black/[0.05]'}`}>
        {!collapsed && (
          ...
          >
            ...
            <span className={`text-lg tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}
      ...
      {!collapsed && (
        <div className="px-3 py-3">
          <p className={`text-[10px] uppercase tracking-wider px-2 mb-1.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}
          ...
          >
            <div
              className="w-7 h-7 rounded-lg flex items-center justify-center text-white flex-shrink-0 shadow"
              style={{ background: currentTenant.color, fontSize: '11px', fontWeight: 700 }}
            >
      ...
      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        {!collapsed && (
          <p className={`text-[10px] uppercase tracking-wider px-2 mb-1.5 mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}
            style={{ fontWeight: 600 }}>
            Navigation
          </p>
        )}
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                size={18}
                className={`relative z-10 flex-shrink-0 ${isActive ? (isDark ? 'text-indigo-400' : 'text-indigo-600') : ''}`}
              />
              {!collapsed && (
              ...
              {isActive && !collapsed && (
                <div className="relative z-10 ml-auto w-1.5 h-1.5 rounded-full bg-indigo-500" />
              )}
            </button>
          )
        })}
          ...
          <div className="pt-3">
            ...
            <div className="flex gap-1 mb-2">
              {(Object.keys(APP_USERS) as (keyof typeof APP_USERS)[]).map((role) => (
                <button
                  key={role}
                  onClick={() => onUserChange(APP_USERS[role])}
            ...
            >
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-white text-xs flex-shrink-0 shadow-md"
                style={{ fontWeight: 700 }}>
                {currentUser.initials}
              </div>
              <div className="flex-1 text-left min-w-0">
                <p className={`text-sm truncate ${isDark ? 'text-slate-200' : 'text-slate-800'}`} style={{ fontWeight: 600 }}>
                  {currentUser.name}
                </p>
              ...
              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] border ${roleBg} ${roleColor}`}
                style={{ fontWeight: 600 }}>
                <RoleIcon size={9} />
                {currentUser.role}
              </div>
    ...
    {/* Tenant dropdown — portaled to body to escape sidebar stacking context */}
    {createPortal(
      <AnimatePresence>
        {tenantDropOpen && tenantRect && (
          <motion.div
            ...
            style={{
              position: 'fixed',
              top: tenantRect.bottom + 6,
              left: tenantRect.left,
              width: tenantRect.width,
              zIndex: 99999,
            }}
          ...
          >
            {TENANTS.map((tenant) => (
              <button
                key={tenant.id}
                onClick={() => { onTenantChange(tenant); setTenantDropOpen(false) }}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 transition-colors ${
                  isDark ? 'hover:bg-white/[0.06] text-slate-200' : 'hover:bg-black/[0.04] text-slate-800'
                }`}
              >
                <div
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-white flex-shrink-0"
                  style={{ background: tenant.color, fontSize: '11px', fontWeight: 700 }}
                >
                  {tenant.initials}
                </div>
            ...
            <button className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${
              isDark ? 'hover:bg-white/[0.06] text-indigo-400' : 'hover:bg-indigo-50 text-indigo-600'
            }`}>
              <Layers size={14} />
    ...
    {/* User menu — portaled to body to escape sidebar stacking context */}
    {createPortal(
      <AnimatePresence>
        {userMenuOpen && userRect && (
          <motion.div
            ...
            style={{
              position: 'fixed',
              bottom: window.innerHeight - userRect.top + 4,
              left: userRect.left,
          ...
          >
            ...
            <button className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm transition-colors ${
